...
public class TestDoubleLogin {
	...
	public void test() throws UnknownHostException, IOException {
		...
		for (int i = 0; i < 2; i++) {
			...
			// 其他消息，不管
			if (ProtoHead.ENetworkMessage.OFFLINE_SYNC != PacketFromClient.getMessageType(resultBytes))
				continue;
			// System.err.println(NetworkMessage.getMessageType(resultBytes));

			// 回复服务器
			client1.writeToServer(PacketFromClient.packMessage(ProtoHead.ENetworkMessage.OFFLINE_SYNC_VALUE,
					PacketFromClient.getMessageID(resultBytes), new byte[]{}));
